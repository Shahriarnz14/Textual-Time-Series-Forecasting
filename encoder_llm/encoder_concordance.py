...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_data_directory', type=str, default='ordered_l33train', help='Directory containing training data')
    parser.add_argument('--test_data_directory',  type=str, default='ordered_l33test', help='Directory containing Test data')
    ...
    concordance_data, _ = prepare_dataset(train_timeseries_data, K, H)
    concordance_train, concordance_val = train_test_split(concordance_data, test_size=0.2, shuffle=True)

    # Preprocess and tokenize data once
    concordance_train = preprocess_and_tokenize_data(concordance_train, tokenizer, args.max_length, args.batch_size)
    concordance_val = preprocess_and_tokenize_data(concordance_val, tokenizer, args.max_length, args.batch_size)
    ...
    concordance_test, _ = prepare_dataset(test_timeseries_data, K, H)
    concordance_test = preprocess_and_tokenize_data(concordance_test, tokenizer, args.max_length, args.batch_size)

    # Evaluate on validation set
    _, concordance_index_score = evaluate_model_concordance(
            model, concordance_test
        )
    print(f"Test Concordance Index: {concordance_index_score:.4f}")
